...
@router.post(
    ...
    dependencies=[],
    description="Khởi tạo giao dịch chuyển tiền giữa các tài khoản ngân hàng",
)
async def initiate_money_transfer(
    transfer_data: TransferRequestSchema,
    current_user = Depends(require_permission(PermissionChoicesSchema.INITIATE_MONEY_TRANSFER)),
    session: AsyncSession = Depends(get_session),
    idempotency_key: str = Header(
        description="Idempotency Key cho request chuyển tiền"
    ),
) -> TransferResponseSchema:
    """
    ...
    try:
        ...
        if not idempotency_key:
            raise HTTPException(
                ...
                detail={
                    "status": "error",
                    "message": "Idempotency-Key header không hợp lệ.",
                },
            )
        ...
        try:
            ...
            logger.error(f"Gửi email OTP thất bại: {e}")

        # 5. Tạo response Pending cho client
        response = TransferResponseSchema(
            status="pending",
            message="Vui lòng nhập mã OTP đã được gửi tới email của bạn để hoàn tất giao dịch.",
            data={
        ...
        logger.error(f"Khởi tạo giao dịch chuyển tiền thất bại: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"status": "error", "message": "Khởi tạo giao dịch chuyển tiền thất bại."},
        )
...
@router.post(
    ...
    dependencies=[Depends(require_permission(PermissionChoicesSchema.COMPLETE_MONEY_TRANSFER))],
    description="Hoàn tất giao dịch chuyển tiền giữa các tài khoản ngân hàng",
)
async def complete_money_transfer(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Gửi thông báo giao dịch thất bại: {e}")

        # 3. Trả response giao dịch thành công cho client
        return TransferResponseSchema(
            status="success",
            message="Giao dịch chuyển tiền hoàn tất thành công.",
            data={
                "reference": transaction.reference,
                "amount": format_currency(
                    transaction.amount,
        ...
        # Bắt lỗi hệ thống không mong muốn
        logger.error(f"Hoàn tất giao dịch chuyển tiền thất bại: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"status": "error", "message": "Hoàn tất giao dịch chuyển tiền thất bại."},
        )
